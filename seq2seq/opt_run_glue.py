...
from seq2seq.approxlinear.glue_trainer import GLUETrainer
...
from seq2seq.third_party.models.opt import OPTForCausalLM, OPTForSequenceClassification, OPTConfig
...
def main():
    ...
    for key in datasets.keys():
        datasets[key] = datasets[key].add_column("task", [data_args.task_name] * datasets[key].num_rows)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        result["average_metrics"] = np.mean(list(result.values())).item()
        return result
    ...
    trainer = GLUETrainer(
        ...
        # evaluation_metrics=TASK_TO_METRICS[data_args.task_name]
    )
