...
from seq2seq.approxlinear.config import ApproxLinearConfig
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, adapter_config=None, lora_config=None, approx_config: ApproxLinearConfig=None):
        super().__init__()
        self.bitfit = adapter_config.bitfit if adapter_config is not None else False 
        ...
        if approx_config is not None:
            ...
            self.o = ApproxLinear(self.inner_dim, self.d_model, bias=False if not self.bitfit else True, 
                                  config=approx_config)
            self.matmul_op = Approxmatmul_4D(config=approx_config)
