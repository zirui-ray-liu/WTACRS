...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, adapter_config=None, lora_config=None, approx_config=None):
        ...
        if lora_config is not None:
            ...
            if approx_config is not None and approx_config.q_sampling:
                self.q = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True, approx_config=approx_config)
            else:
                self.q = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True, approx_config=None)

            if approx_config is not None and approx_config.k_sampling:
                self.k = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True, approx_config=approx_config)
            else:
                self.k = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True, approx_config=None)

            if approx_config is not None and approx_config.v_sampling:
                self.v = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True, approx_config=approx_config)
            else:
    ...
    def forward(
        ...
        if self.use_lora:
            ...
            self.k.forward = partial(self.k.forward, task=task)
