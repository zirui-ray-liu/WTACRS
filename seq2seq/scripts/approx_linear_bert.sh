...
source scripts/env_approx_bert.sh
...
python scripts/update_scripts_for_given_input.py $update_file_name inplace_layernorm   bool false $update_file_name
...
CUDA_VISIBLE_DEVICES=$1 python run_glue.py  $update_file_name
