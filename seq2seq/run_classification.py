...
from seq2seq.utils.utils_bart import get_adapter_config, modify_model_after_init
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, ClassificationTrainingArguments,
    ...
    if "fisher" in adapter_args.load_side_pretrained_weights:
        ...
        model = BartForSequenceClassification.from_pretrained(model_args.model_name_or_path)
        temp_tokenizer = AutoTokenizer.from_pretrained(model_args.model_name_or_path)
        ...
        # )
        importance_measure = compute_fisher(model, train_dataset, data_collator, num_samples=adapter_args.samples_for_fisher)

        if adapter_args.use_bottleneck:
            from seq2seq.pruning.pruning_methods_bart import pruning_bottleneck
            method = pruning_bottleneck
        elif adapter_args.create_side_lm:
            from seq2seq.pruning.pruning_methods_bart import pruning_with_residual
            method = pruning_with_residual
        else:
            from seq2seq.pruning.pruning_methods_bart import pruning_without_residual
            method = pruning_without_residual

        print(method.__name__)

        pruned_state_dict = method(model, temp_tokenizer, adapter_args.task_reduction_factor, importance_measure=importance_measure)

    if "bart" in adapter_args.load_side_pretrained_weights:
        from transformers.models.bart.modeling_bart import BartForSequenceClassification
        model = BartForSequenceClassification.from_pretrained(model_args.model_name_or_path)
        temp_tokenizer = AutoTokenizer.from_pretrained(model_args.model_name_or_path)
        ...
        pruned_state_dict = method(model, temp_tokenizer, adapter_args.task_reduction_factor)

    # Initialize the model 
    from seq2seq.third_party.models.bart.modeling_bart_encoder import BartForSequenceClassification
    ...
    if adapter_args.load_side_pretrained_weights == "bart":
    ...
    elif "fisher" in adapter_args.load_side_pretrained_weights or "bart" in adapter_args.load_side_pretrained_weights:
        state_dict = model.state_dict()
        for n, p in model.named_parameters():
            if "side_block" in n:
                infer_n = n.split(".")
                ...
                if adapter_args.weight_scale:
                    keep_ratio = (state.shape[0] / state_dict[infer_n].shape[0]) ** 0.5
                    print(keep_ratio)
                else:
                    keep_ratio = 1
