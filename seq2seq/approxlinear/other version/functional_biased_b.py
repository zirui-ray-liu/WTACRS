...
class approxmatmul_4D_only_bw(Function):
    ...
    def forward(ctx, A, B, ratio, deter_ratio, minimal_k, scheme, batch_dim_use_same_indices=True):
        ...
        if ratio == 1.0 or grad_A is None or grad_B is None:
            ...
            B_top_k_trans, top_k_indices_B = subsample_4D_B_by_norm(grad_A, B.transpose(3, 2), None, ratio, deter_ratio, minimal_k,
                                            batch_dim_use_same_indices=batch_dim_use_same_indices)

            A_top_k, top_k_indices_A = subsample_4D_B_by_norm(grad_B, A, None, ratio, deter_ratio, minimal_k,
                                            batch_dim_use_same_indices=batch_dim_use_same_indices)
    ...
    def backward(ctx, grad_output):
        ...
        if top_k_indices_B is None: # B_top_k.shape[3] == B_shape[3]:
            ...
            if top_k_indices_B.ndim == 1:
                ...
                # grad_A = grad_A_deter + grad_A_stoc * B_weight_stoc
        ...
        if top_k_indices_A is None: # A_top_k.shape[2] == A_shape[2]:
            ...
            if top_k_indices_A.ndim == 1:
                ...
                # grad_B = grad_B_deter + grad_B_stoc * A_weight_stoc
...
def subsample_4D_B_by_norm(A, B, weight, sample_ratio, deter_ratio, minimal_k, batch_dim_use_same_indices: bool = True):
    ...
    if batch_dim_use_same_indices:
        ...
        top_k_values_deter, top_k_indices_deter = topk(norm_mult, deter_k, dim=1, largest=True)
        top_k_indices_deter, _ = torch.sort(top_k_indices_deter, dim=1)
        top_k_prob = (top_k_values_deter.sum(dim=1) + 1e-8) / (norm_mult.sum(dim=1) + 1e-8)
        norm_mult.scatter_(dim=1, index=top_k_indices_deter, src=(top_k_values_deter * 0.))
        residual_prob = (norm_mult + 1e-8) / (norm_mult.sum(dim=1).unsqueeze(dim=1) + 1e-8)
        ...
        top_k_indices_stoc = torch.multinomial(residual_prob, stoc_k, replacement=False)
        ...
        top_k_indices = torch.cat([top_k_indices_deter, top_k_indices_stoc], dim=2)
        top_k_indices, _ = torch.sort(top_k_indices, dim=2)
        B_ = torch.scatter(B, dim=2, index=top_k_indices_stoc.unsqueeze(-1).expand(b, h, stoc_k, d), src=B_top_k_stoc_unbiased)
        B_top_k = torch.gather(B_, dim=2, index=top_k_indices.unsqueeze(-1).expand(b, h, k, d))

    return B_top_k, top_k_indices

    # stoc_weight = (1 - top_k_prob).view(b, h, 1, 1)
    # print(B_top_k_deter.shape, top_k_indices_deter.shape)
    # return B_top_k_deter, top_k_indices_deter, B_top_k_stoc_unbiased, top_k_indices_stoc, stoc_weight
...
class approxlinear_only_bw(Function):
    ...
    def forward(ctx, input, weight, bias, ratio, deter_ratio, minimal_k, scheme, batch_dim_use_same_indices=True):
        ...
        if ratio == 1.0 or scale is None:
            ctx.saved = input, None, None, None, None, weight, bias
        else:
            subsampled_input_deter, top_k_indices_deter, subsampled_input_stoc, top_k_indices_stoc, weight_stoc \
                = subsample_3D_input_by_norm(input, scale, ratio, deter_ratio, minimal_k, batch_dim_use_same_indices)

            ctx.saved = subsampled_input_deter, top_k_indices_deter, subsampled_input_stoc, top_k_indices_stoc, \
                        weight_stoc, weight, bias
    ...
    def backward(ctx, grad_output):
        ...
        weight_stoc, weight, bias = ctx.saved
        ...
        if top_k_indices_deter is None:
            ...
            grad_weight = grad_weight_deter + grad_weight_stoc * weight_stoc
        ...
        del subsampled_input_deter, top_k_indices_deter, subsampled_input_stoc, top_k_indices_stoc, weight_stoc, weight, bias
...
def subsample_3D_input_by_norm(input, grad_norm, sample_ratio, deter_ratio, minimal_k, batch_dim_use_same_indices: bool =True):
    ...
    if k == in_features:
        return input, None, None, None, None
    ...
    top_k_values_deter, top_k_indices_deter = topk(norm_mult, deter_k, largest=True)
    top_k_indices_deter, _ = torch.sort(top_k_indices_deter)

    top_k_prob = (top_k_values_deter.sum() + 1e-8) / (norm_mult.sum() + 1e-8)
    norm_mult[top_k_indices_deter] = 0
    residual_prob = (norm_mult + 1e-8) / (norm_mult.sum() + 1e-8)
    ...
    input_top_k_stoc_unbiased = (input_top_k_stoc + 1e-8) / (residual_prob_sampled + 1e-8) / stoc_k
    stoc_weight = (1 - top_k_prob)

    # print(input_top_k_deter.shape, top_k_indices_deter.shape)
    return input_top_k_deter, top_k_indices_deter, input_top_k_stoc_unbiased, top_k_indices_stoc, stoc_weight
