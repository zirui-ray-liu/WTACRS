...
def subsample_4D_B_by_norm(A, B, weight, scheme):
    ...
    if scheme.batch_dim_use_same_indices:
        ...
        if stoc_k == 0:
            ...
            return B_top_k_deter, top_k_indices_deter, None, None
        ...
        prob[mask, :] = 1.
        prob.scatter_(dim=1, index=top_k_indices_deter, src=(top_k_values_deter * 0.))
        mask = mask.view(b, h, 1, 1).type(torch.int)

        top_k_indices_deter = top_k_indices_deter.view(b, h, -1)
        B_top_k_deter = torch.gather(B, dim=2, index=top_k_indices_deter.unsqueeze(-1).expand(b, h, deter_k, d))

        top_k_indices_stoc = torch.multinomial(prob, stoc_k, replacement=scheme.sample_replacement)
        ...
        if a_col_norms.sum() == 0.:
            ...
            if scheme.sample_replacement:
                prob_sampled = torch.gather(prob.view(b, h, n), dim=2, index=top_k_indices_stoc)
                B_top_k_stoc_unbiased = (1 - mask) * B_top_k_stoc_unbiased / prob_sampled.unsqueeze(dim=3) / stoc_k
...
def subsample_3D_input_by_norm(input, grad_norm, scheme):
    ...
    if stoc_k == 0:
        ...
        return input_top_k_deter, top_k_indices_deter, None, None
